...
export function PresenceOverlay(props: {
	...
	const { item, layout, presence, remoteIds, getInitials, getUserColor, expanded, onToggleExpanded } = props;
	...
	return (
		<g transform={`translate(${b.left}, ${b.top}) rotate(${angle}, ${w/2}, ${h/2})`} data-svg-item-id={item.id}>
			{users.length === 1 ? (
				...
				<g onMouseDown={(e)=>e.stopPropagation()}>
					{!expanded ? (
						<g transform={`translate(${w - 12}, ${-12})`} cursor="pointer" onClick={onToggleExpanded}>
						...
						<g>
							...
							<g transform={`translate(${w - 12 - users.length * 26 - 14}, ${-12})`} cursor="pointer" onClick={onToggleExpanded}>
