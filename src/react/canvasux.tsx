...
export function Canvas(props: {
	...
	const { canvasPosition, pan, zoom, beginPanIfBackground, handleHtmlBackgroundMouseDown, handleBackgroundClick } = useCanvasNavigation({ svgRef, presence, setSize });
		...
		>
			...
			<foreignObject x={0} y={0} width="100%" height="100%">
				...
				<div className="relative h-full w-full" onMouseDown={handleHtmlBackgroundMouseDown}
					...
					<ItemsHtmlLayer items={items} canvasPosition={canvasPosition} pan={pan} zoom={zoom} />
				</div>
			</foreignObject>
			...
			<g key={`sel-${selKey}-${motionKey}`} transform={`translate(${pan.x}, ${pan.y}) scale(${zoom})`}>
				{items.map((item) => {
					...
					return (
						<SelectionOverlay key={`wrap-${item.id}`} item={item} layout={layout} presence={presence} zoom={zoom} />
					);
				})}
			</g>
